export default class LiquidGoal {
  constructor(target, options = {}) {
    ...
    const def = {
      ...
      bubbles: { enabled: false, rate: 10, minR: 1.5, maxR: 4, minVy: 20, maxVy: 60, drift: 15, alpha: 0.6, color: 'auto' }
    };
    this.opt = { ...def, ...options };
    if (options.background) this.opt.background = { ...def.background, ...options.background };
    if (options.bubbles) this.opt.bubbles = { ...def.bubbles, ...options.bubbles };
  ...
  setOptions(patch = {}) {
    ...
    if (patch.bubbles) this.opt.bubbles = { ...this.opt.bubbles, ...patch.bubbles };
    const prev = this.opt.value;
    Object.assign(this.opt, patch);
    if ('value' in patch && patch.value !== prev) this._emit('valueChanged', { value: this.opt.value, percent: this.getDisplayedPercent() });
  }
  ...
  setProgress(v) {
    ...
    if (nv !== this.opt.value) {
      this.opt.value = nv;
      this._emit('valueChanged', { value: this.opt.value, percent: this.getDisplayedPercent() });
    }
  }
  updateProgress(d) { this.setProgress((this.opt.value || 0) + (Number(d) || 0)); }
  reset() { this.state.level = 0; this.state.vel = 0; this.setProgress(0); }
  jumpTo(v) { this.setProgress(v); this.state.level = this.opt.value; this.state.vel = 0; }
  animateTo(v, duration = 0.8) {
    const from = this.opt.value;
    const to = Math.max(0, Number(v) || 0);
    this.state.anim = { active: true, t: 0, dur: Math.max(0.001, duration), from, to };
  }
  toDataURL(type = 'image/png', quality) { return this.el.toDataURL(type, quality); }
  getState() {
    return {
      goal: this.opt.goal,
  ...
  getDisplayedPercent() {
    return Math.round(Math.max(0, this.state.level / (this.opt.goal * this.opt.norm100)) * 100);
  }
  ...
  _emit(type, detail) { this.el.dispatchEvent(new CustomEvent(type, { detail })); }
  ...
  _spawnBubbles(dt, baseY) {
    ...
    const w = this.el.width, h = this.el.height, d = this.dpr;
    ...
    while (this._bubbleSeed >= 1) {
      ...
      const vy = -this._rand(this.opt.bubbles.minVy, this.opt.bubbles.maxVy) * d;
      const drift = this.opt.bubbles.drift * d;
      const phase = Math.random() * Math.PI * 2;
      const alpha = this._clamp(this.opt.bubbles.alpha, 0, 1);
      const color = this.opt.bubbles.color === 'auto' ? this._shade(this.opt.gradientFrom, 0.4) : this.opt.bubbles.color;
  ...
  _step(dt){
    ...
    const base = this._lerp(h*0.96, h*0.06, Math.max(0, s.level/(o.goal*o.norm100)));
    this._spawnBubbles(dt, base);

    if (Math.abs(s.level - prevLevel) > 0.001) this._emit('progress', { value: s.level, percent: this.getDisplayedPercent() });
    const curPercent = this.getDisplayedPercent();
    if (prevPercent < 100 && curPercent >= 100) this._emit('goalReached', { value: s.level, percent: curPercent });
  }
